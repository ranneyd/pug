...
let main;
{
    ...
    function compileBody(str, options){
      ...
      var filtersSet = {};
      Object.keys(main.filters).forEach(function (key) {
        filtersSet[key] = main.filters[key];
      });
      if (options.filters) {
    ...
    function handleTemplateCache (options, str) {
      ...
      if (options.cache && main.cache[key]) {
        return main.cache[key];
      } else {
        if (str === undefined) str = fs.readFileSync(options.filename, 'utf8');
        var templ = main.compile(str, options);
        if (options.cache) main.cache[key] = templ;
        return templ;
      }
    }
    ...
    main.compile = function(str, options){
      var options = options || {}
    ...
    main.compileClientWithDependenciesTracked = function(str, options){
      var options = options || {};
     ...
     * @api public
     */
    main.compileClient = function (str, options) {
      return main.compileClientWithDependenciesTracked(str, options).body;
    };
     ...
     * @api public
     */
    main.compileFile = function (path, options) {
      options = options || {};
    ...
    main.render = function(str, options, fn){
    ...
    main.renderFile = function(path, options, fn){
      // support callback API
      if ('function' == typeof options) {
        fn = options, options = undefined;
      }
      if (typeof fn === 'function') {
        var res
        try {
          res = main.renderFile(path, options);
        } catch (ex) {
          return fn(ex);
        }
        return fn(null, res);
    ...
    main.compileFileClient = function(path, options){
      var key = path + ':client';
      options = options || {};
      ...
      var out = main.compileClient(str, options);
      if (options.cache) main.cache[key] = out;
      return out;
    };
    ...
    main.__express = function(path, options, fn) {
      if(options.compileDebug == undefined && process.env.NODE_ENV === 'production') {
        options.compileDebug = false;
      }
      main.renderFile(path, options, fn);
    }
}
return main();